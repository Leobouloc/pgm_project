...
A = rand(num_dicts, num_dicts); % State transition
...
A = A ./ repmat(sum(A, 2), 1, num_dicts);
...
for xxx = 1:num_iterations
    ...
    for t = 1:T-1
        for qt=1:num_dicts
            temp = (log(A(qt, :)') + P3_mat(T-t+1, :)' + log_beta(T-t+1, :)')'; % +1 ???
            [max_val, max_ind] = max(temp);
            temp_but_max = [temp(1:max_ind-1), temp(max_ind+1:end)];
    ...
    for t=1:T-1
        for qt = 1:num_dicts
            ...
            %end
            for qt_plus_one = 1:num_dicts
                p_qt_qt_plus_one(t, qt, qt_plus_one) = 1/sum(exp(log_alpha(t,:) + log_beta(t,:) - log_alpha(t,qt) - log_beta(t+1, qt_plus_one) - P3_mat(t+1, qt_plus_one)))...
                         * A(qt, qt_plus_one);
            end
        end
